...
${DIVIDER}
  [1] Cast vote
  [2] Display current voting value
  [3] Exit
${'─'.repeat(62)}
> `;
      ...
      case '1':
        try {
          const contract = await api.withStatus('Deploying voting contract', () =>
            api.deploy(providers, { privateVoting: 0 }),
          );
 ...
 * can vote on the contract or query its current value.
 */
const mainLoop = async (providers: VotingProviders, walletCtx: api.WalletContext, rli: Interface): Promise<void> => {
  ...
  while (true) {
    ...
    switch (choice.trim()) {
      case '1':
        try {
          await api.withStatus('Casting vote', () => api.increment(votingContract));
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
        ...
        break;
      case '2':
        await api.displayVotingValue(providers, votingContract);
        break;
      case '3':
        return;
      default:
 ...
 *   4. Enter the contract deploy/join and voting interaction loop
...
export const run = async (config: Config, _logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  try {
    ...
    if (dockerEnv !== undefined) {
      ...
      if (config instanceof UndeployedConfig) {
        config.indexer = mapContainerPort(env, config.indexer, 'voting-indexer');
        config.indexerWS = mapContainerPort(env, config.indexerWS, 'voting-indexer');
        config.node = mapContainerPort(env, config.node, 'voting-node');
        config.proofServer = mapContainerPort(env, config.proofServer, 'voting-proof-server');
      }
    }
