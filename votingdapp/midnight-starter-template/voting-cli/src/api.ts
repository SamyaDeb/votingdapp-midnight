...
const votingCompiledContract = CompiledContract.make('voting', Counter.Contract).pipe(
...
export const joinContract = async (
  ...
  const votingContract = await findDeployedContract(providers, {
    ...
    initialPrivateState: { privateVoting: 0 },
  });
...
export const increment = async (votingContract: DeployedVotingContract): Promise<FinalizedTxData> => {
  logger.info('Incrementing...');
  const finalizedTxData = await votingContract.callTx.increment();  

  return finalizedTxData.public;
};

export const displayVotingValue = async (
  providers: VotingProviders,
  votingContract: DeployedVotingContract,
): Promise<{ votingValue: bigint | null; contractAddress: string }> => {
  const contractAddress = votingContract.deployTxData.public.contractAddress;
  const votingValue = await getVotingLedgerState(providers, contractAddress);
  if (votingValue === null) {
    logger.info(`There is no voting contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Current voting value: ${Number(votingValue)}`);
  }
  return { contractAddress, votingValue };
};
